...
Function Create-ErrorObject([string]$status, [string]$message, [string] $action, [string] $type){
    ...
    if (Test-Path $configPath) { #physical server exists, get the properties
                        ...
                        switch ($portType) {
                            "EnetServerPort"{
                                ...
                                if(!$portObject.model.contains("Flex")){
                                    $script:configErrors += Create-ErrorObject "Critical" "$($portObject.model) in bay $($physicalBlade.Bay) is unsupported" "If possible replace this adapter with a supported model" "Servers and IO modules"
                                    }
                                    #added by Thabet Handle green
                                    else 
                                    {
                                        $script:configErrors += Create-ErrorObject "Green" "$($portObject.model) in bay $($physicalBlade.Bay) is unsupported" "Green" "Servers and IO modules"
                                    }
                                }
                            "FcServerPort"{
                                $thisPort.portInfo = $portObject
        ...
        If($aServers -notcontains ($physicalBlade.BladeInfo.name).toLower()){
            If ($physicalBlade.BladeInfo.name -match "Gen9"){
                $script:configErrors += Create-ErrorObject "Warning" "$($physicalBlade.BladeInfo.name) in bay $($physicalBlade.Bay) is not supported for migration with an assigned profile" "Unassign the profile from the server prior to migration." "Servers and IO modules"
            }
            else {
                $script:configErrors += Create-ErrorObject "Critical" "$($physicalBlade.BladeInfo.name) in bay $($physicalBlade.Bay) is not supported for OneView management" "Remove the server from the enclosure or import the enclosure for monitoring." "Servers and IO modules"
            }
        }
        #added by Thabet Handle green
         else 
         {
            $script:configErrors += Create-ErrorObject "Green" "$($physicalBlade.BladeInfo.name) in bay $($physicalBlade.Bay) is not supported for OneView management" "Green" "Servers and IO modules"
         }
...
function Get-ProfilesConnectionMap {
    ...
    ForEach ($c in $conns){
        ...
        if($c.GUID -match ":ec") {
            ...
            #PXE Boot Setting
            Write-Log "Checking PXE boot setting on connection."
            if ($c.PxeBootSetting -eq 1) {}
        }    
        ...
        if ($c.GUID -match ":fc") {
            ...
            if($c.NumFcBootTargetConfigs -gt 0){
                ...
                if ($c.BootPriority -eq 2) { #primary
...
Function Confirm-NetSet {
    ...
    $aServers += "HP Proliant BL460c G7", "Proliant BL460c G7" , "HP Proliant BL460c Gen8", "Proliant BL460c Gen8"
    ...
    $aServers += "HP Proliant BL660c Gen8", "Proliant BL660c Gen8"
...
################  BEGIN MAIN SCRIPT
    ...
    if ($continue) {
        ...
        # VCEM
        if($vcDomainInfo.UlmData -contains "VcemFirmwareMode"){
        ...
        ForEach ($mod in $enetModules){
            If($mod.name -notmatch "Flex") {$script:configErrors += Create-ErrorObject "Critical" "$($mod.name) at $($mod.GUID) is not supported for management." "Upgrade the interconnect hardware." "Servers and IO modules"}
            #added by Thabet Handle green
             else 
             {
                $script:configErrors += Create-ErrorObject "Green" "$($mod.name) at $($mod.GUID) is not supported for management." "Green" "Servers and IO modules"
             }
        }

        #FC Modules
        ForEach ($mod in $fcModules){
           if($mod.name -notmatch "8Gb") {$script:configErrors += Create-ErrorObject "Critical" "$($mod.name) at $($mod.GUID) is not supported for management." "Upgrade the interconnect hardware." "Servers and IO modules"}
            #added by Thabet Handle green
            else 
             {
                $script:configErrors += Create-ErrorObject "Green" "$($mod.name) at $($mod.GUID) is not supported for management." "Green" "Servers and IO modules"
